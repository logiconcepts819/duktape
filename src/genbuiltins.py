...
def metadata_convert_lightfuncs(meta):
		...
		for p in o['properties']:
			...
			if targ.has_key('magic'):
				try:
					...
					#print('Failed to resolve magic: ' + str(e))
	...
	print('Converted %d built-in function properties to lightfuncs' % num_converted)
	...
	for o in meta['objects']:
		...
		for v in o['properties']:
			...
			if isinstance(val, dict):
				...
				# FIXME: either skip lightfuncs (with warning) for RAM built-ins, or add --rom-only option?
